...
> 把"性格"变成可计算、可旋转、可克隆的东西。**v0.2.5** | Skill API · Core Engine · REST API · Web UI
...
┌─────────────────────────────────────────────────┐
│                 外部消费者                         │
│  Agnes Skill │ FastAPI REST │ Gradio Web UI     │
└──────┬──────────┬──────────┬───────────────────┘
       │          │          │
       ▼          ▼          ▼
┌─────────────────────────────────────────────────┐
...
详细用法见 [`docs/SKILL.md`](docs/SKILL.md)。
...
> **跨平台支持**：本系统可在 Windows、macOS、Linux 上运行。部分路径/命令可能需根据操作系统微调。
...
```
渊/
├── src/personality_core/    # 核心算法（引擎、记忆、情感、安全、Trait预测、Skill封装）
│   ├── engine.py            # 总控引擎
│   ├── embedder.py          # 文本→向量嵌入
...
- [`docs/SKILL.md`](docs/SKILL.md) — Skill 集成说明（推荐入口）
